...
| **`effects-ui.js`** | Effect toggle buttons next to the theme control; persists **`site-effect-cursor-invert`** (opt-in, default off) and **`site-effect-hero-magnifier`** (opt-out, default on); dispatches **`site-effects-changed`**. |
